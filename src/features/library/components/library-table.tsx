...
import { LIBRARY_CATEGORY, PRICE_REGION, type LibraryCategory } from '../constants/library.constants'
import type { LibraryFilters, LibraryItem } from '../types/library.types'

const CATEGORY_VARIANT: Record<LibraryCategory, 'default' | 'secondary' | 'outline'> = {
  material: 'default',
  labor: 'secondary',
  equipment: 'outline'
}
              ...
              <TableBody>
                {data.items.map((item) => (
                  <TableRow key={item.id}>
                    ...
                    <TableCell>
                      <Badge variant={CATEGORY_VARIANT[item.category]}>{t(`category.${item.category}`)}</Badge>
                    </TableCell>
                    <TableCell>
                      <Badge variant='outline'>{t(`region.${item.region}`)}</Badge>
                    </TableCell>
                    ...
                    <TableCell className='text-right'>
                      <div className='flex justify-end gap-1'>
                        ...
                        <Button
                          ...
                          onClick={() => toast.success(t('form.deleted', { name: item.name }))}
                        >
...
function HistoryPopover({ item, locale }: { item: LibraryItem; locale: Locale }) {
  const t = useTranslations('library')
      ...
      <PopoverContent align='end' className='w-64'>
        ...
        <ul className='space-y-1.5'>
          {[...item.priceHistory].reverse().map((pt, i) => (
            <li key={pt.date} className='flex items-center justify-between text-sm'>
              <span className='text-muted-foreground'>
                {formatDate(pt.date, locale)}
                {i === 0 ? ` · ${t('history.current')}` : ''}
              </span>
              <span className='tabular-nums'>{formatCurrency(pt.price, locale)}</span>
            </li>
          ))}
        </ul>
      </PopoverContent>
