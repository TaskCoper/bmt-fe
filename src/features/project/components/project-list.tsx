  ...
  return (
    <div className='space-y-4'>
      ...
      {isLoading ? (
        ...
        <>
          <div className='rounded-lg border' data-fetching={isFetching ? '' : undefined}>
            <Table>
              <TableHeader>
                <TableRow>
                ...
                {data.items.map((project) => (
                  <TableRow key={project.id}>
                    <TableCell>
                      ...
                      {project.description ? (
                        <div className='text-muted-foreground truncate text-sm'>{project.description}</div>
                      ) : null}
                    </TableCell>
                    <TableCell>
                      <Badge variant={STATUS_VARIANT[project.status]}>{t(`status.${project.status}`)}</Badge>
                    </TableCell>
