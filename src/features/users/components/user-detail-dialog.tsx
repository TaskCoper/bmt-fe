...
export function UserDetailDialog({ trigger, user }: { trigger: ReactNode; user: UserRecord }) {
  ...
  const rows: { label: string; value: ReactNode }[] = [
    ...
    { label: t('columns.role'), value: t(`roles.${user.role}`) },
    {
      ...
      value: <Badge variant={user.status === 'active' ? 'success' : 'secondary'}>{t(`status.${user.status}`)}</Badge>
    },
    {
