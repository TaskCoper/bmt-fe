...
import { Search, MoreHorizontal, Lock, LockOpen, KeyRound, Eye } from 'lucide-react'
...
export function UserTable() {
  ...
  const resetPassword = (user: UserRecord) => toast.success(t('actions.resetToast', { name: user.name }))
              ...
              <TableBody>
                {data.items.map((user) => (
                  <TableRow key={user.id}>
                    ...
                    <TableCell>
                      <Badge variant={ROLE_VARIANT[user.role]}>{t(`roles.${user.role}`)}</Badge>
                    </TableCell>
                    <TableCell>
