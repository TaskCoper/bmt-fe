...
export function EstimateDetail({ section, tier }: { section: string; tier: string }) {
  ...
  const tItems = useTranslations('studio.items') as unknown as (key: string) => string
    ...
    return (
      <div className='relative min-h-screen'>
        ...
        <div className='relative mx-auto max-w-4xl p-6'>
          <div className='glass-panel-strong text-muted-foreground p-10 text-center text-sm'>{t('subtitle')}</div>
        </div>
      </div>
    )
  }
  ...
  const tierLabel = section === 'rough' ? sectionLabel : tPkg(`${tier}.name` as `${PackageTier}.name`)
    ...
    <div className='relative min-h-screen'>
      ...
      <div className='relative mx-auto max-w-4xl p-6'>
        <div className='glass-panel-strong space-y-6 p-6 sm:p-8'>
          ...
          <BudgetProgressBar total={breakdown.total} target={project.data.budget} />
            ...
            <Table>
              <TableHeader>
                <TableRow className='border-glass-border bg-background/50 backdrop-blur hover:bg-transparent'>
                  ...
                  <TableHead className='w-24 text-right'>{t('colDetail')}</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {tierData.items.map((item) => (
                  <TableRow key={item.name} className='border-glass-border hover:bg-background/40'>
                    <TableCell className='font-medium'>{tItems(item.name)}</TableCell>
                    <TableCell className='text-muted-foreground tracking-tight tabular-nums'>
                      {item.quantity} {tUnit(item.unit as 'm2' | 'm3' | 'kg' | 'set')}
                    </TableCell>
                    <TableCell className='text-muted-foreground text-sm'>{tItems(item.material)}</TableCell>
          ...
          <p className='text-muted-foreground text-right text-sm tracking-tight tabular-nums'>
            {formatCurrency(tierData.total, locale)} · {formatCurrency(breakdown.total, locale)}
          </p>
        </div>
      </div>
            ...
            <>
              ...
              <div className='space-y-5 px-4 pb-6 text-sm'>
                <div className='space-y-1'>
                  <p className='text-muted-foreground text-xs font-medium uppercase'>{t('material')}</p>
                  <p>{tItems(selected.material)}</p>
                </div>
                <div className='space-y-1'>
                  <p className='text-muted-foreground text-xs font-medium uppercase'>{t('alternatives')}</p>
                ...
                <div className='space-y-1'>
                  <p className='text-muted-foreground text-xs font-medium uppercase'>{t('method')}</p>
