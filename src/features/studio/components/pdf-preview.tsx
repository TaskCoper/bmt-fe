    ...
    >
      {opt.coverPage ? (
        <Page label={tp('cover')}>
          <div className='flex h-full flex-col items-center justify-center gap-2 text-center'>
            ...
            <p className='text-sm font-semibold'>{data.name || t('untitled')}</p>
          </div>
        </Page>
      ) : null}
      ...
      {opt.drawings ? (
        <Page label={tp('drawings')}>
          <div className='border-glass-border bg-background/40 grid h-24 place-items-center rounded-lg border'>2D</div>
        </Page>
      ) : null}
