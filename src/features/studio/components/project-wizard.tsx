...
const STEP_COMPONENTS = [StepCreate, StepSpace, StepDesign, StepResult, StepRender, StepExport] as const
...
export function ProjectWizard() {
  ...
  const hasFloorImage = useWizardStore((s) => s.data.images.some((i) => i.floor >= 0))

  const stepKey = WIZARD_STEPS[stepIndex]!
  const StepComponent = STEP_COMPONENTS[stepIndex]!
  const isFirst = stepIndex === 0
  const isLast = stepIndex === WIZARD_STEPS.length - 1
  const canAdvance = stepIndex === 0 ? nameFilled : stepIndex === 1 ? areaFilled && hasFloorImage : true
    ...
    <div className='grid gap-6 lg:grid-cols-[260px_1fr]'>
      ...
      <div className='min-w-0 space-y-6'>
        <div className='space-y-1'>
          <h2 className='text-xl font-semibold'>{tSteps(`${stepKey}.title`)}</h2>
          <p className='text-muted-foreground text-sm'>{tSteps(`${stepKey}.description`)}</p>
        </div>

        <StepComponent />

        <div className='flex items-center justify-between border-t pt-6'>
          <Button variant='outline' onClick={back} disabled={isFirst} className={isFirst ? 'invisible' : undefined}>
      ...
      <Dialog open={pendingNav !== null} onOpenChange={(o) => !o && cancelNav()}>
        <DialogContent>
          <DialogHeader>
