...
export function EstimateTabs({ categories, budget }: { categories: EstimateCategory[]; budget: BudgetBreakdown }) {
  const t = useTranslations('studio.estimate')
  ...
  const tUnit = useTranslations('studio.unit') as unknown as (key: string) => string
            ...
            <TabsContent key={cat.id} value={cat.id} className='space-y-4'>
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead className='text-right'>{t('col.quantity')}</TableHead>
                      ...
                      >
                        <TableRow className='hover:bg-muted/60 cursor-help'>
                          ...
                          <TableCell className='text-right tabular-nums'>
                            {formatNumber(item.quantity, locale)} {tUnit(item.unit)}
                          </TableCell>
              ...
              <div className='space-y-2'>
                <div className='flex items-center justify-between text-sm'>
                  ...
                  <span className='font-semibold tabular-nums'>{formatCurrency(cat.total, locale)}</span>
                </div>
                <Progress value={pct} />
                <p className='text-muted-foreground text-right text-xs'>{t('budgetUsage', { pct })}</p>
              </div>
            </TabsContent>
          )
        })}
