...
import { PACKAGE_TIERS, projectStepPath, type PackageTier } from '../constants/studio.constants'
...
function DetailLink({
  ...
  return (
    <Button variant='ghost' size='sm' asChild>
      <Link href={detailPath(projectId, section, tier)} target='_blank' rel='noopener noreferrer'>
 ...
 */
export function EstimateTable({ projectId, sections }: { projectId: string; sections: EstimateSection[] }) {
  ...
  const renderTierRows = (section: EstimateSection, key: 'finishing' | 'interior') => (
    <div className='space-y-2'>
      {PACKAGE_TIERS.map((tier: PackageTier) => {
        ...
        return (
          <div
            key={tier}
            className={cn('glass-card flex flex-wrap items-center gap-3 p-4', selected && 'glass-selected')}
          >
            <button
              type='button'
              role='radio'
            ...
            >
              ...
              <span className='min-w-0'>
                <span className='block text-sm font-medium'>{tPkg(`${tier}.name`)}</span>
                <span className='text-muted-foreground block text-xs'>{tPkg(`${tier}.desc`)}</span>
              </span>
            </button>
            <span className='text-sm font-semibold tracking-tight tabular-nums'>
              {formatCurrency(sectionTotal(section, tier), locale)}
            </span>
            <DetailLink projectId={projectId} section={key} tier={tier} label={t('viewDetail')} />
          </div>
        )
      })}
    </div>
        ...
        {rough ? (
          <div className='border-glass-border bg-background/40 flex flex-wrap items-center justify-between gap-3 rounded-xl border p-4 backdrop-blur-sm'>
            ...
            <div className='flex items-center gap-3'>
              ...
              <DetailLink projectId={projectId} section='rough' tier='single' label={t('viewDetail')} />
            </div>
          </div>
        ) : null}
      </TabsContent>
