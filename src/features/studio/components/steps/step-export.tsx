    ...
    <div className='space-y-8'>
      <StepSection title={t('contentTitle')} description={t('contentHint')}>
        <div className='grid gap-3 sm:grid-cols-2'>
          {SECTION_KEYS.map((key) => (
            <label key={key} className='hover:bg-muted/50 flex cursor-pointer items-center gap-3 rounded-lg border p-3'>
              <Checkbox checked={options[key] as boolean} onCheckedChange={(v) => setExportOption(key, Boolean(v))} />
          ...
          {EXPORT_LANGUAGES.map((lang) => (
            <button
              ...
              className={cn(
                ...
                options.language === lang ? 'bg-background shadow-sm' : 'text-muted-foreground hover:text-foreground'
              )}
            >
              {t(`language.${lang}`)}
            </button>
      ...
      <StepSection title={t('shareTitle')} description={t('shareHint')}>
        ...
        <div className='flex flex-wrap gap-3'>
          ...
          <Button variant='outline' onClick={() => toast.success(t('emailSent'))}>
