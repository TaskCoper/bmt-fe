...
interface StudioState {
  ...
  createProject: (input: {
    name: string
    constructionType: WizardData['constructionType']
    note: string
  }) => string
...
export const useWizardStore = create<StudioState>()(
  persist(
    (set, get) => ({
      ...
      patch: (partial) =>
        set((s) =>
          withCurrent(s, (snap) => ({ data: { ...snap.data, ...partial } })),
        ),

      setStyle: (style) =>
        set((s) =>
          withCurrent(s, (snap) => ({ data: { ...snap.data, style } })),
        ),
      ...
      regenerate: async () => {
        ...
        if (!id) return
        const snap = get().projects[id]
        if (!snap || snap.regenCount >= MAX_REGENERATIONS || get().isGenerating)
          return
        set((s) =>
          withCurrent(s, (cur) => ({ regenCount: cur.regenCount + 1 })),
        )
        await get().generate()
      },

      clearResult: () =>
        set((s) => withCurrent(s, () => ({ result: null, regenCount: 0 }))),
          ...
          withCurrent(s, (snap) =>
            snap.result
              ? {
                  result: {
                    ...snap.result,
                    renders: snap.result.renders.map((r) =>
                      r.id === rid ? { ...r, favorite: !r.favorite } : r,
                    ),
                  },
                }
              : {},
...
export function useCurrentProject(): ProjectSnapshot | null {
  return useWizardStore((s) =>
    s.currentId ? (s.projects[s.currentId] ?? null) : null,
  )
}
