...
import { DEFAULT_LEADS_PAGE_SIZE, LEAD_STATUS } from '../constants/leads.constants'
...
function applyFilters(filters: LeadFilters): LeadRecord[] {
  ...
  if (filters.search) {
    ...
    items = items.filter(
      (l) => l.name.toLowerCase().includes(q) || l.phone.toLowerCase().includes(q) || l.email.toLowerCase().includes(q)
    )
  }
...
export const mockLeadsApi = {
  async list(filters: LeadFilters): Promise<PaginatedResponse<LeadRecord>> {
    await mockDelay()
    return paginate(applyFilters(filters), filters.page, DEFAULT_LEADS_PAGE_SIZE)
  },
